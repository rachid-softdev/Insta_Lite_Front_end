...
import { extractFileNameFromUrl, fetchFile } from '../../../core/utils/file-utils';
...
export class ImageListComponent implements OnInit, OnDestroy {
  ...
  private generateRandomFileName(): string {
    ...
    return `images_${timestamp}_${randomString}`;
  }
  ...
  public onClickDownloadAllImages(): void {
    ...
    this.getImages.forEach((image, index) => {
      ...
      zip.file(fileName, this.fetchImage(imageUrl), { binary: true });
    });
  ...
  // Fonction utilitaire pour récupérer les données binaires de l'image
  private fetchImage(url: string): Promise<ArrayBuffer> {
